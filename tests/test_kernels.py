...
class TestAutoKernelCreation(object):
    ...
    def test_constructor_kernels(self,se_kernel):
        gpobj = GPR1D.KernelConstructor(se_kernel.name)
        assert isinstance(gpobj,GPR1D._Kernel), '%r is not Kernel object!' % (gpobj)

    def test_constructor_warping_functions(self,gibbs_constant_kernel):
        gpobj = GPR1D.KernelConstructor(gibbs_constant_kernel.name)
        assert isinstance(gpobj,GPR1D._Kernel), '%r is not Kernel object!' % (gpobj)

    def test_constructor_operators(self,sum_kernel):
        gpobj = GPR1D.KernelConstructor(sum_kernel.name)
        assert isinstance(gpobj,GPR1D._Kernel), '%r is not Kernel object!' % (gpobj)

    def test_reconstructor_kernels(self,se_kernel):
        kernel_pars = np.hstack((se_kernel.hyperparameters,se_kernel.constants)).flatten()
        gpobj = GPR1D.KernelReconstructor(se_kernel.name,kernel_pars)
    ...
    def test_reconstructor_warping_functions(self,gibbs_inverse_gaussian_kernel):
        ...
        gpobj = GPR1D.KernelReconstructor(gibbs_inverse_gaussian_kernel.name,kernel_pars)
    ...
    def test_reconstructor_operators(self,sum_kernel):
        ...
        gpobj = GPR1D.KernelReconstructor(sum_kernel.name,kernel_pars)
        assert gpobj == sum_kernel


@pytest.mark.usefixtures("constant_kernel")
class TestConstantKernel(object):
...
class TestNoiseKernel(object):
    ...
    ref_dcov = np.zeros(x1.shape)
    ref_ddcov = np.zeros(x1.shape)
    ref_hdcov = [np.diag(2.0 * np.ones(test_x_vector.shape))]
...
class TestLinearKernel(object):
    ...
    ref_hdcov = [np.atleast_2d([[0.0,0.0],[0.0,4.0]])]
...
class TestPolyOrderKernel(object):
...
class TestSquareExponentialKernel(object):
    ...
    ref_hdcov = [np.atleast_2d([[2.0,0.27067056647],[0.27067056647,2.0]]),
...
class TestRationalQuadraticKernel(object):
    ...
    ref_hdcov = [np.atleast_2d([[2.0,0.37186886416],[0.37186886416,2.0]]),
...
class TestMaternHalfIntegerKernel(object):
    ...
    ref_hdcov = [np.atleast_2d([[2.0,0.27732043828],[0.27732043828,2.0]]),
...
class TestGibbsKernelWithConstant(object):
    ...
    ref_hdcov = [np.atleast_2d([[2.0,1.21306131943],[1.21306131943,2.0]]),
...
class TestGibbsKernelWithInverseGaussian(object):
    ...
    ref_hdcov = [np.atleast_2d([[2.0,0.21474364904],[0.21474364904,2.0]]),
...
class TestSumOperationKernel(object):
...
class TestProductOperationKernel(object):
    ...
    ref_hdcov = [np.atleast_2d([[0.0,0.0],[0.0,0.0]]),
